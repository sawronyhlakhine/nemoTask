...
class Layout {
    ...
    function render() {
        ...
        $CI->styles = $CI->styles ?? $CI->config->item('styles');
        $CI->scripts = $CI->scripts ?? $CI->config->item('scripts');
        $CI->meta = $CI->meta ?? $CI->config->item('meta');
        $CI->parts = $CI->parts ?? [];
        if (!isset($CI->layout)) {
            $CI->layout = "default";
        }
        if ($CI->layout != false) {
            ...
            if (count($CI->scripts) > 0) {  // Массив со скриптами
                foreach ($CI->scripts as $script) {
                    $scripts .= "<script type='text/javascript' src='" . base_url() . "/public" . $script . ".js'></script>";
                }
            }
            if (count($CI->styles) > 0) {   // Массив со стилями
                foreach ($CI->styles as $style) {
                    $styles .= "<link rel='stylesheet' type='text/css' href='" . base_url() . "/public" . $style . ".css' />";
                }
            }
